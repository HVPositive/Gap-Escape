...
public class ControlManager : MonoBehaviour {
	...
	public SteamVR_Action_Boolean gapAction;
	...
	void Update () {
        ...
        if  ( cj.connectedBody!=null){
			...
			if (dropAction.GetState( GetSide() ) ) {
            ...
            else if ( forwardAction.GetState( GetSide() ) ){
            ...
            else if (backwardAction.GetState( GetSide() ) ){
		...
		if (pointerAction.GetState(  GetSide() ))
		{
			MakePointer();
		}
		else
		...
		if (pointer.enabled)
		{
			...
			if (Physics.Raycast(landingRay, out hit))
			{
				...
				if (hit.collider.tag == "Level" )
				{
					...
					if (confirmAction.GetState( GetSide() ) )
					{	
					...
					if ((confirmAction.GetState( GetSide() ) && openDoor.GetComponent<SECTR_Door>().IsFullyOpen() ) )
					{	
					...
					if (gapAction.GetStateDown( GetSide() ) ){
	...
	void MakePointer()
	{
		...
		pointer.SetPosition(0, controllerLoc.transform.position);
		pointer.SetPosition(1, controllerLoc.transform.forward * 100);
    ...
    //Get the approriate controller
	SteamVR_Input_Sources GetSide(){
		if (side == "LeftHand")
